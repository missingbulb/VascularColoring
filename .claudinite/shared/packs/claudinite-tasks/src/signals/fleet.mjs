...
import { isDormant } from '../contract/dormancy.mjs';
import { actionsEnv } from '../world/actions.mjs';
import { readCommit } from '../world/github.mjs';
...
async function localPacksChangedInWindow(gh, fullName, defaultBranch, sinceIso) {
  ...
  for (const c of list) {
    if (!c?.sha) continue;
    const detail = await readCommit(gh, fullName, c.sha);
...
export function makeFleetGh(env = actionsEnv()) {
