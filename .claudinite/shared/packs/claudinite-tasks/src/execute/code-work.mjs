// The CODE-WORK phase of task execution: the
...
// code-work communicates only through the repository.
//
...
// log, and "communicate only through the repository" is untouched. It is echoed
// LIVE rather than dumped at exit for the case that needs it most — a worker SIGKILLed
// at its timeout, whose buffered output would otherwise die with it.

import { existsSync, rmSync, readFileSync } from 'node:fs';
import { join } from 'node:path';
import { tmpdir } from 'node:os';
import { startProcess } from '../world/processes.mjs';
...
export function runCodeWork(command, {
  ...
  return new Promise((resolve) => {
    ...
    const child = startProcess(command, { cwd: taskDir, env, shell: true, detached: true });
    let stdout = '';
    let stderr = '';
    let timedOut = false;
...
// The conditional-handoff signal (E4). A task with
...
// (no code→agent data channel). The hand-off condition must name work
...
//
// This is the one thing that crosses the code→agent boundary as data (the named
