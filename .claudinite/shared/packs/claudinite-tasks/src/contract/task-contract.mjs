// The task declaration contract - the single
...
// worker-file rule gives agent_instructions.
...
// The defaults live in a module with no imports, so the
...
// WHO MINTS AN OCCURRENCE, and WHAT MUST HOLD once one exists.
...
// them. Loaders normalize once; everything
...
export function normalizeTaskDeclaration(decl) {
  ...
  delete out.$schema;
  // THE CADENCE-SPELLING DOOR. `due:<cadence>` is the same
...
// What a task's run does to PULL REQUESTS - the write ceiling and the target in one word, resolved once by the
...
// 'anything', or a list of diff classes the merge policy evaluates the actual diff
// against.
...
// `requeue` is the safe-side default for sweep-shaped
...
// The signal-collector vocabulary. A task collects only the union
...
export function validateTaskDeclaration(raw, terms = new Map()) {
  ...
  // The signal union is DERIVED from the expression's terms, each of which names
  // what it reads, so the collector can never disagree with what the gate actually
  // consults - a declared list could.
  ...
  // never one inferred from the shape of its conditions: an inferred one cannot tell a
  // task nobody asks from one whose conditions merely happen to look unaskable.
  ...
  // ONE MECHANISM. `preconditions` - a list of named conditions, all of
  // which must hold - is the only gate a task declares. A `precondition`
  // function is rejected rather than tolerated: two forms would mean every
  ...
  // A task's cadence is one of its own conditions, read off its own run history, so
  // `frequency` is rejected by NAME rather than ignored, and naming the term to write, so a declaration carrying
  ...
  // OPTIONAL: a task may require nothing, and then every occurrence of
   ...
   * so it is fenced rather than waved through: the shape check accepts
  ...
  // Code-work - OPTIONAL, and declared in one of two
  ...
  // The WRAPPED form: a module beside task.json exporting `worker`, which the
  // runner's own entry point imports and calls. The runner supplies what a raw
  // worker must do itself - reading the CLAUDINITE_* environment, the exit code, the
  // failure line, the timing, the queue's markers - so the module is the work and
  // nothing else. A file name, never a command: the runner
  ...
  // `schedule_after` - ordering, declared. A list of `<pack>/<task>` ids this
  ...
  // `on_interrupt` - the ack-early/ack-late dial. Most of this fleet's
  ...
  // `invocation_endpoint` - a NAME, never a URL. The repo's config
  ...
  // The repo Actions secrets this task's code work needs configured. Purely
  ...
  // Execution bound - an agentic task MUST
  ...
  // code-work - a `none` task with no code-work does nothing.
