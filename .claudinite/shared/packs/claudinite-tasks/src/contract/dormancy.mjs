...
import { TASKS_PACK_ID, DORMANT_CONFIG_KEY } from '../../public/task-constants.mjs';

export { TASKS_PACK_ID };
...
export function dormancyErrors(config) {
  ...
  return [{
    what: `"${DORMANT_CONFIG_KEY}" on the "${TASKS_PACK_ID}" pack entry must be true or false, got ${JSON.stringify(value)}`,
