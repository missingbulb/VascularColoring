// The tasks the ENGINE itself ships, as opposed to
...
// against the repo: the pack root is wherever this file was vendored to — the canon's
// own tree, or `.claudinite/shared/` in a member — and asking the module cannot get
// that wrong the way a two-root probe can. `queue/tasks/` is the frozen path a work
// item's machine block names, so it is addressed from the pack root, two up.
...
// access blesses an issue they did not open - the blessing is the
